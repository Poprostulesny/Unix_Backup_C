...
/*checking whether the directory exists and is empty. if it doesnt exist it
   creates it. return -1 when is not a directory return 0 when it is not empty
   or didnt exist
*/
...
*/
/*Function that gets the full path for source and target, and current whole
 * path, and returns the path from target to path in target */
char *get_path_to_target(const char *source, const char *target,
                         const char *path) {
  int s, t, p;
  s = strlen(source);
  t = strlen(target);
...
void copy_file(const char *path1, const char *path2) {
  ...
  if (last_sep != NULL) {
    *last_sep = '\0';
    ...
    make_path(dest_dir);
    /* make_path builds all parents, but not the last component; ensure the leaf
     * dir exists */
...
void copy(const char *source, const char *dest, char *backup_source,
          char *backup_target) {
  struct stat st;
...
void create_empty_files(const char *dest_path, node_tr *target,
                        node_sc *source_node, void *ctx) {
  (void)target;
...
void copy_files(const char *dest_path, node_tr *target, node_sc *source_node,
                void *ctx) {
...
void attribs(const char *dest_path, node_tr *target, node_sc *source_node,
             void *ctx) {
  (void)target;
  const char *src_path = (const char *)ctx;
  copy_permissions_and_attributes(src_path, dest_path);
...
void delete_multi(const char *dest_path, node_tr *target, node_sc *source_node,
                  void *ctx) {
  (void)target;
  (void)ctx;
...
void move_all(const char *dest_path, node_tr *target, node_sc *source_node,
              void *ctx) {
...
int backup_walk(const char *path, const struct stat *s, int flag,
                struct FTW *ftw) {
