...
#ifndef INOTIFY_MASK
#define INOTIFY_MASK                                                           \
  (IN_CREATE | IN_CLOSE_WRITE | IN_DELETE | IN_DELETE_SELF | IN_MOVE_SELF |    \
...
int backup_walk_inotify_init(const char *path, const struct stat *s, int flag,
  ...
  if (flag == FTW_D) {
    ...
    add_wd_node(&_target_node->watchers, wd, _source_friendly, _source, path,
                suffix);
    free(suffix);
  }
  return 0;
...
#ifdef DEBUG
  printf("Doing an initial backup of %s to %s...\n", source_node->source_full,
         target_node->target_full);
#endif
...
void debug_printer(char *type, int wd, char *source_friendly, char *path,
                   char *event_name, int is_dir) {
#ifdef DEBUG
  printf("Watch %d in folder %s (%s) saw %s on %s (is_dir: %d)\n", wd,
         source_friendly, path, type, event_name, is_dir);
#endif
}
...
void event_handler(node_sc *source_node, node_tr *target_node) {
  ...
  while (1) {
    ...
    if ((event->mask & IN_CREATE) && is_dir) {
      debug_printer("IN_CREATE", event->wd, wd_node->source_friendly,
                    wd_node->path, event->name, is_dir);
    ...
    if ((event->mask & IN_CLOSE_WRITE) && !is_dir) {
      debug_printer("IN_CLOSE_WRITE", event->wd, wd_node->source_friendly,
                    wd_node->path, event->name, is_dir);
      char *dest_path = concat(2, target_node->target_full, suffix);
      if (dest_path != NULL) {
    ...
    if (event->mask & IN_DELETE) {
      debug_printer("IN_DELETE", event->wd, wd_node->source_friendly,
                    wd_node->path, event->name, is_dir);
      char *dest_path = concat(2, target_node->target_full, suffix);
      if (dest_path != NULL) {
    ...
    if (event->mask & IN_MOVED_FROM) {
      debug_printer("IN_MOVED_FROM", event->wd, wd_node->source_friendly,
                    wd_node->path, event->name, is_dir);
      add_move_event(&target_node->mov_dict, event->cookie, event->full_path, 1,
    ...
    if (event->mask & IN_MOVED_TO) {
      debug_printer("IN_MOVED_TO", event->wd, wd_node->source_friendly,
                    wd_node->path, event->name, is_dir);
      add_move_event(&target_node->mov_dict, event->cookie, event->full_path, 0,
                     source_node, target_node);
      // jezeli istnieje in_moved_from z cookiesem matchujacym nasz ->
      // przenosimy jezeli po MOV_TIME s nie ma, kopiujemy plik
    }
    if (event->mask & IN_ATTRIB) {
      debug_printer("IN_ATTRIB", event->wd, wd_node->source_friendly,
                    wd_node->path, event->name, is_dir);
      char *dest_path = concat(2, target_node->target_full, suffix);
      if (dest_path != NULL) {
    ...
    if (event->mask & IN_DELETE_SELF) {
      debug_printer("IN_DELETE_SELF", event->wd, wd_node->source_friendly,
                    wd_node->path, event->name, is_dir);
      char *dest_path = concat(2, target_node->target_full, suffix);
      if (dest_path != NULL) {
    ...
    if (event->mask & IN_MOVE_SELF) {
      debug_printer("IN_MOVE_SELF", event->wd, wd_node->source_friendly,
                    wd_node->path, event->name, is_dir);
      if (strcmp(event->full_path, source_node->source_full) == 0) {
        kill(getpid(), SIGUSR2);
      }
    ...
    if (event->mask & IN_IGNORED) {
      debug_printer("IN_IGNORED", event->wd, wd_node->source_friendly,
                    wd_node->path, event->name, is_dir);
