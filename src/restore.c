...
// przechodzi przez backup i kopiuje zmienione/brakujące pliki do source, static
// bo dostepne tylko wewnatrz pliku
static int restorer(const char *path, const struct stat *s, int flag,
                    struct FTW *ftw) {
  ...
  if (flag == FTW_D) {
    ...
    if (dst_exists == -1 && errno == ENOENT) {
      ...
      if ((S_ISREG(s->st_mode) && !S_ISREG(st.st_mode)) ||
          (S_ISLNK(s->st_mode) && !S_ISLNK(st.st_mode))) {
        recursive_deleter(dest_path);
        need_copy = 1;
      } else if (S_ISREG(s->st_mode)) {
        if (s->st_size != st.st_size ||
            s->st_mtim.tv_sec != st.st_mtim.tv_sec ||
            s->st_mtim.tv_nsec != st.st_mtim.tv_nsec) {
          need_copy = 1;
        }
      } else if (S_ISLNK(s->st_mode)) {
        if (s->st_mtim.tv_sec != st.st_mtim.tv_sec ||
            s->st_mtim.tv_nsec != st.st_mtim.tv_nsec) {
          need_copy = 1;
        }
      }
...
static int delete_missing(const char *path, const struct stat *s, int flag,
                          struct FTW *ftw) {
