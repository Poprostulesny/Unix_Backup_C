...
// Function instantiating inotify recursively in a given source for current target
int backup_walk_inotify_init(const char* path, const struct stat* s, int flag, struct FTW* ftw);

// Handler of initial backup events. Creates inotify instance for the target if necessary
