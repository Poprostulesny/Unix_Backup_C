...
/* Use SA_RESTART for handlers that should not interrupt blocking syscalls
 * (e.g., getline). */
void sethandler_restart(void (*f)(int), int sigNo) {
  struct sigaction act;
  memset(&act, 0, sizeof(struct sigaction));
  act.sa_handler = f;
...
void unblock_sigint_sigterm() {
  ...
  if (sigaddset(&set, SIGINT) == -1 || sigaddset(&set, SIGTERM) == -1 ||
      sigaddset(&set, SIGUSR2) == -1 || sigaddset(&set, SIGUSR1) == -1 ||
      sigaddset(&set, SIGCHLD) == -1 || sigaddset(&set, SIG_ACK) == -1) {
    ERR("sigaddset");
  }
...
void child_loop(node_sc *source_node, node_tr *target_node, char *buf) {
  ...
  if (sigaddset(&child_unblock, SIGUSR1) == -1 ||
      sigaddset(&child_unblock, SIGUSR2) == -1) {
    ERR("sigaddset child");
  }
...
void spawn_child_for_target(node_sc *source_node, node_tr *target_node,
  ...
  if (pid == 0) {
    child_loop(source_node, target_node, buf);
  }
...
void handle_end_for_target(node_sc *source_node, node_tr *target_node) {
  ...
  if (list_target_delete(&source_node->targets, target_node->target_friendly) &&
      child_count > 0) {
    child_count--;
  }
}
...
int validate_target(node_sc *to_add, char *tok) {
  // If an element is already a target or a source of another element in the
  // list, dont add it to the targets and print an error message.
  if (tok[0] == '/') {
    printf("Invalid path\n");
    return 0;
  }
...
int parse_targets(node_sc *to_add, char *buf) {
  ...
  int cnt = 0;
  while (tok != NULL) {
    // If the directory is not empty, write and continue parsing, is_empty_dir
    // ensures that it is empty or creates it if it doesnt exist
    ...
    spawn_child_for_target(to_add, new_node, buf);
#ifdef DEBUG
    printf("Full source: '%s', friendly: '%s'\n", to_add->source_full,
           to_add->source_friendly);
...
int take_input(char *buf) {
  ...
  if (source_full == NULL) {
    printf("Source directory doesn't exist '%s': %s\n", tok, strerror(errno));
    return 0;
  }
  ...
  if (source_elem == NULL) {
    ...
    if (source_elem->source_friendly == NULL ||
        source_elem->source_full == NULL) {
      ERR("strdup failed");
    }
    ...
    fprintf(stderr, "NEW node_sc %p, source='%s'\n", (void *)source_elem,
            source_elem->source_full);
#endif
  }

  // Parsing the list of targets
  int cnt = parse_targets(source_elem, buf);

  // If we have added something, and we didn't have the element before add the
  // new node to the list of active whatchers
...
void list_sources_and_targets() {
  ...
  while (current != NULL) {
    printf("Source: '%s', real path: '%s' ,", current->source_friendly,
           current->source_full);
...
void inotify_jobs(node_sc *source_node, node_tr *target_node) {
  ...
  inotify_reader(target_node->inotify_fd, &target_node->watchers,
                 &target_node->events);
...
void restore(char *tok) {
  ...
  puts("Stopping all backup operations");
  ...
  if (expected_waiting > 0) {
    ...
    sigdelset(&suspend_mask, SIG_ACK);
...
void input_handler() {
  ...
  while (1) {
    ...
    // input in the form add <source path> <target path> with multiple target
    // paths
    if (strcmp(tok, "add") == 0) {
      if ((k = take_input(buff)) <= 0) {
        // do smth
        puts("Invalid syntax\n");
        continue;
      }
    }
    // input in the form end <source path> <target path> with multiple target
    // paths
    ...
    // input in the form restore <source path> <target path> with singular
    // target path
...
int main() {
  ...
  sethandler_restart(sigack_parent, SIG_ACK);
  /* Restart getline instead of returning EINTR when a child exits. */
  sethandler_restart(sigchld_handler, SIGCHLD);
